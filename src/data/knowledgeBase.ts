...
export const kbEntries: KBEntry[] = [
  ...
  { t:'RAG范式演变全景', c:'SimpleRAG(检索+生成)60%→CorrectiveRAG(自检纠错)70%→Self-RAG(反思打分)75%→AgenticRAG(多步推理+工具)85%→GraphRAG(知识图谱)88%→SpeculativeRAG(推测检索)90%→FusionRAG(混合融合)92%。', g:['RAG','范式','演变'] },
  { t:'SimpleRAG基础模式', c:'三段式:文档分块→向量索引→查询检索→LLM生成。Chunk大小512-1024 tokens，重叠10-20%。局限:无关检索影响准确率。', g:['RAG','基础','SimpleRAG'] },
  { t:'CorrectiveRAG自纠错', c:'检索后先自检文档相关性。不相关自动触发Web搜索补充。相关度过低拒绝回答。检索质量提升30%。', g:['RAG','自纠错','CRAG'] },
  { t:'Self-RAG反思增强', c:'模型生成时输出反思Token:Retrieve/IsRel/IsSup/IsUse。检索相关性+支持性+有用性三重反思。比基础RAG提升20%准确性。', g:['RAG','反思','Self-RAG'] },
  { t:'AgenticRAG多步推理', c:'RAG+Agent融合:检索不是一次性而是多轮交互。Agent规划检索策略→自动拆分子问题→链式检索→交叉验证。复杂问题准确率85%+。', g:['RAG','Agent','多步推理'] },
  { t:'SpeculativeRAG推测检索', c:'小模型快速生成假设→大模型验证+检索。推测解码思路迁移到RAG。检索效率提升2-3倍。', g:['RAG','推测','加速'] },
  { t:'FusionRAG混合融合', c:'多源融合:向量检索+关键词+图检索+SQL。RAG-Fusion多查询聚合排序。综合准确率92%。企业级RAG终极方案。', g:['RAG','融合','终极'] },
  { t:'AI四层架构体系', c:'L1编排层(任务分解+资源分配)→L2交付层(代码/文案/设计)→L3数据底座层(RAG/知识图谱/Embedding)→L4治理安全层(安全审查/质量检测/审计)。每一层由专门Agent负责。', g:['架构','四层','Agent'] },
  { t:'A2A协议详解', c:'Agent-to-Agent:Google跨平台Agent通信标准。Agent Card声明能力+Task对象表示任务+SSE Streaming推送状态+JSON-RPC over HTTP传输。与MCP互补:A2A管Agent间通信，MCP管Agent-工具通信。', g:['A2A','协议','Google'] },
  { t:'Agent Card能力声明', c:'A2A核心概念:每个Agent发布自己的Card(能力、技能、端点URL)。类似OpenAPI规范但面向Agent。支持动态发现和自动匹配。', g:['A2A','AgentCard','协议'] },
  { t:'MCP协议核心三原语', c:'Model Context Protocol标准接口:tools/list(可执行工具)+resources/read(可访问资源)+prompts/get(预设提示模板)。JSON-RPC 2.0协议。200+官方社区Server。', g:['MCP','协议','工具'] },
  { t:'AI进化六大方向', c:'规模(更大模型参数+数据)→智能(更强推理+思维链)→架构(Transformer→MoE→Mamba→混合)→经济(降成本+提效率+端侧)→交互(多模态+实时+Agent自主)→生命(自进化+自我维护+自我复制)。', g:['进化','方向','AI'] },
  { t:'SOP全栈开发技能包', c:'完整流程:需求分析→技术选型→项目脚手架→后端API→数据库设计→前端界面→测试→部署。适用:Web应用/SaaS平台/管理系统。', g:['SOP','全栈','开发'] },
  { t:'SOP API迁移技能包', c:'四阶段:旧API文档解析→生成接口映射表→编写适配器→流量切换+回滚方案。关键:兼容性测试全覆盖+灰度发布。', g:['SOP','API','迁移'] },
  { t:'SOP安全审计技能包', c:'五步骤:资产盘点→漏洞扫描(SAST/DAST)→渗透测试→修复验证→合规报告。覆盖OWASP Top10+CWE Top25。', g:['SOP','安全','审计'] },
  { t:'SOP RAG引擎搭建', c:'七步骤:文档解析(PDF/DOCX/HTML)→智能分块(SemanticChunking)→Embedding(BGE/M3E)→向量索引(Milvus/Qdrant)→检索(HyDE+BM25)→重排序(Cohere/ColBERT)→生成+引用。', g:['SOP','RAG','搭建'] },
  { t:'Token费率速查', c:'GPT-4o:$2.5/1M in $10/1M out。DeepSeek-V3:￥1/1M in ￥2/1M out。Claude3.5Sonnet:$3/1M in $15/1M out。Gemini2.0Flash:免费。Groq:免费。Qwen Turbo:￥0.3/1M。', g:['Token','费率','成本'] },
  { t:'Agent四大协作模式', c:'Sequential管道(串行分工)、Hierarchical主从(主管分派子Agent)、Debate辩论(多方辩论达成共识)、Swarm群体(大量Agent涌现式协作)。模式选择取决于任务复杂度。', g:['Agent','协作','模式'] },
  { t:'LangChain vs LangGraph', c:'LangChain:Chain抽象快速搭建→简单线性流程。LangGraph:状态图→循环/条件/分支→复杂Agent逻辑。LangChain适合原型，LangGraph适合生产。', g:['LangChain','LangGraph','框架'] },
  { t:'Function Calling机制', c:'LLM输出结构化JSON调用外部API。核心:定义函数Schema(name/description/parameters)→注入系统提示→模型返回function_call→客户端执行→结果回传。工具选择不消耗生成Token。', g:['FunctionCalling','工具','API'] },
  { t:'Computer Use模式', c:'Claude可直接操控鼠标键盘浏览网页执行操作。截图→视觉理解→生成动作→执行→循环。Agent从API走向GUI操控。在WebArena和OSWorld评测领先。', g:['ComputerUse','GUI','Claude'] },
  { t:'Devin软件工程全貌', c:'首个认证AI软件工程师。架构:终端+浏览器+编辑器+Planner规划器。SWE-bench独立修复真实GitHub issues成功率22%。工作流:理解issue→定位代码→生成patch→运行测试→提交PR。', g:['Devin','软件工程','Agent'] },
  { t:'Gorilla工具选择', c:'伯克利Function Calling评测榜。LLM在大量工具中选择正确API并生成参数。核心挑战:工具描述歧义+参数类型推断+复杂约束。GPT-4准确率85%+开源模型Llama3达70%。', g:['Gorilla','工具','评测'] },
  { t:'WebArena网页Agent评测', c:'模拟真实网站(电商/社交/论坛/GitLab/地图)评估Agent浏览能力。任务:购物/发帖/代码管理/信息检索。当前最高约40%成功率。', g:['WebArena','Agent','评测'] },
  { t:'Mem0持久用户记忆', c:'开源记忆层。自动从对话提取用户偏好→存储→跨会话检索。支持:用户画像/长期偏好/历史话题。API:add/search/update/delete。嵌入+向量检索+LLM总结。', g:['Mem0','记忆','用户'] },
  { t:'DSPy自动Prompt优化', c:'斯坦福框架:用编程而非手写方式构建Prompt。签名(Signature)定义输入输出→模块(Module)组合逻辑→优化器(Optimizer)自动调参。比手写Prompt提升20-50%质量。', g:['DSPy','Prompt','优化'] },
  { t:'SWE-agent架构解析', c:'普林斯顿开源代码Agent。ACI Agent-Computer Interface:专用命令(Ls/Find/Edit/Goto/Python)简化Agent-代码交互。SWE-bench 18%+成功率。核心:简洁命令空间+代码视图+lint反馈。', g:['SWE-agent','代码','Agent'] },
  { t:'PocketFlow轻量Agent框架', c:'百行代码级Agent框架。核心:Node(节点)+Flow(流程)+SharedStore(共享状态)。节点可嵌套→支持复杂多Agent编排。极致简洁适合学习Agent核心原理。', g:['PocketFlow','Agent','框架'] },
  { t:'Agno高性能Agent框架', c:'性能优先Agent框架。Cython加速+零拷贝状态传递。支持:多Agent+记忆+知识+RAG+工具。比LangGraph快10000倍。适合高吞吐场景。', g:['Agno','Agent','高性能'] },
]
